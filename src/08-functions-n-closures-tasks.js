...
function getPolynom(...arg) {
  return (x) => {
    ...
    let pow = 0;
    const res = arg.map((a, key) => {
      let count = x;
      if (x === 0 && key !== arg.length - 1) return a * x;
      if (key === arg.length - 1) count = 1;
      if (key < arg.length) pow = arg.length - key - 1;
      count **= pow;
      return a * count;
    });
...
function retry(func, attempts) {
  ...
  return () => {
    for (let i = 0; count < attempts + 1; i += 1) {
      try {
        return func();
      } catch (error) {
        sum();
...
function logger(func, logFunc) {
  return (...arg) => {
    const glue = arg.map((a) => JSON.stringify(a)).join(',');
    const str = (a) => `${func.name}(${glue}) ${a}`;
    const starts = str('starts');
    const ends = str('ends');
    logFunc(starts);
    const res = func.call(null, ...arg);
    logFunc(ends);
    return res;
  };
}
 ...
 *   partialUsingArguments(fn, 'a','b','c','d')() => 'abcd'
 */
function partialUsingArguments(f, ...arg) {
  return (...res) => f(...arg.concat(res));
}
